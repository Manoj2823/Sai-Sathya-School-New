...
$mode = $_GET['mode'] ?? 'view'; // view | edit

// ── EDIT SAVE (user can only edit, not add/delete) ────────────
if ($action === 'edit_save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($admin_emails)) {
        ...
        $subject = "Hero Slide Modified by User: $username";
        $body = "Hello Admin,\n\nThe user '$username' has modified a Hero Slide (ID: $id - $title).\n\nPlease log in to the admin panel to review the changes.\n\nBest regards,\nSai Schools System";
...
$edit_row = null;
if (isset($_GET['edit'])) {
    $edit_res = mysqli_query($conn, "SELECT * FROM hero_slides WHERE id=" . (int) $_GET['edit']);
...
// User details for profile panel
...
<head>
    ...
    <title>Hero Slides – User Panel</title>
...
<body>
    ...
    <div class="content">
        ...
        <div class="user-notice">ℹ️ <strong>User Access:</strong> You can only <strong>Edit</strong> existing slides.
            Add and Delete are admin-only.</div>

        <?php if ($edit_row): ?>
            <!-- ── EDIT FORM ── -->
            <div class="card">
                <div class="card-title">✏️ Edit Slide — #<?= $edit_row['id'] ?></div>
                <form method="POST" action="slides.php"
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="tag" value="<?= htmlspecialchars($edit_row['tag'] ?? '') ?>" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="btn_text" value="<?= htmlspecialchars($edit_row['btn_text'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>🔗 Button Link</label>
                            <input type="text" name="btn_link" value="<?= htmlspecialchars($edit_row['btn_link'] ?? '') ?>">
                        </div>
                        <div class="form-group">
        ...
        <!-- ── SLIDES TABLE ── -->
        <div class="card">
            <div class="card-title">📋 All Hero Slides</div>
                ...
                <table>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($slides_res)): ?>
                            <tr>
                                <td><?= $row['id'] ?></td>
                                <td><img src="../<?= htmlspecialchars($row['image_path']) ?>" class="slide-thumb"
                                        onerror="this.src='https://placehold.co/72x46?text=No+Img'"></td>
                                <td>
                                    <strong><?= htmlspecialchars($row['title']) ?></strong><br>
                                    <span
                                        style="color:#94a3b8;font-size:.76rem;"><?= htmlspecialchars($row['tag']) ?></span>
                                </td>
                                <td>
                                    <span style="font-size:.8rem;"><?= htmlspecialchars($row['btn_text']) ?></span><br>
                                    <span
                                        style="color:#94a3b8;font-size:.74rem;"><?= htmlspecialchars($row['btn_link']) ?></span>
                                </td>
                                ...
                                <td>
                                    <a href="slides.php?edit=<?= $row['id'] ?>" class="btn-sm btn-edit-sm">✏️ Edit</a>
