...
// ── UPLOAD HELPER ────────────────────────────────────────────
function uploadImage($file_key, $subfolder = 'images')
{
    ...
    // Save to main site images folder (one level up)
    $dest_dir = '../' . $subfolder . '/';
    if (!is_dir($dest_dir))
    ...
    if (move_uploaded_file($_FILES[$file_key]['tmp_name'], $dest)) {
        return $subfolder . '/' . $filename;   // relative path for DB
    }
...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $upload = uploadImage('image_file', 'images');
    if (is_array($upload) && isset($upload['error'])) {
        ...
        $btn_text = mysqli_real_escape_string($conn, trim($_POST['btn_text']));
        $btn_link = mysqli_real_escape_string($conn, trim($_POST['btn_link']));
        $sort_order = (int) ($_POST['sort_order'] ?? 0);

        if ($image_path && $title) {
            mysqli_query($conn, "INSERT INTO hero_slides (image_path,tag,title,description,btn_text,btn_link,sort_order,is_active)
                VALUES ('$image_path','$tag','$title','$desc','$btn_text','$btn_link',$sort_order,1)");
            $msg = '✅ Slide added successfully! It will be shown on the main page.';
            $msg_type = 'success';
        } else {
            $msg = 'Image and Title required.';
            $msg_type = 'error';
        }
    }
}
...
if ($action === 'edit_save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_query($conn, "UPDATE hero_slides SET 
        tag='$tag', title='$title', description='$desc',
...
if ($action === 'toggle' && isset($_GET['id'])) {
    $id = (int) $_GET['id'];
    mysqli_query($conn, "UPDATE hero_slides SET is_active = 1-is_active WHERE id=$id");
    header('Location: slides.php?msg=toggled');
    exit;
}
...
if ($action === 'delete' && isset($_GET['id'])) {
    ...
    header('Location: slides.php?msg=deleted');
    exit;
}

if (isset($_GET['msg'])) {
    $msgs = ['toggled' => 'Slide status changed!', 'deleted' => 'Slide deleted!'];
    $msg = $msgs[$_GET['msg']] ?? '';
    $msg_type = 'success';
}
...
if (isset($_GET['edit'])) {
    ...
    $edit_res = mysqli_query($conn, "SELECT * FROM hero_slides WHERE id=$edit_id");
    $edit_row = mysqli_fetch_assoc($edit_res);
}
$page = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
$limit = 10;
$offset = ($page - 1) * $limit;
$total_rows = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM hero_slides"))[0];
$total_pages = ceil($total_rows / $limit);
$slides_res = mysqli_query($conn, "SELECT * FROM hero_slides ORDER BY sort_order ASC, id ASC LIMIT $limit OFFSET $offset");
...
<head>
    ...
    <style>
        ...
        .page-btn { padding: 6px 12px; border: 1px solid #e2e8f0; border-radius: 6px; text-decoration: none; color: var(--navy); font-size: 0.85rem; font-weight: 700; transition: all 0.2s; background: #fff; }
        ...
        * {
        ...
        .content > * { animation: fadeSlideUp 0.5s ease forwards; opacity: 0; }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="slides.php" class="nav-item active">
    ...
    <div class="main" <?= !$is_admin ? 'style="margin-left:0;"' : '' ?>>
        <header class="topbar">
            <div style="display:flex; align-items:center; gap:16px;">
                ...
                <?php endif; ?>
                <h1>🖼️ Hero Slides Management</h1>
            </div>
            <div style="display:flex; align-items:center; gap:20px;">
                <a href="<?= $dashboard_link ?>" style="color:#64748b;text-decoration:none;font-size:.9rem;">← Dashboard</a>
        ...
        <div class="content">
            ...
            <div class="card">
                <div class="card-title">
                    <?= $edit_row ? '✏️ Edit Slide' : '➕ Add New Slide' ?>
                </div>
                <form method="POST" enctype="multipart/form-data" action="slides.php">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <div class="hint">JPG/PNG/WEBP · Max
                                5MB<?= $edit_row ? ' · Leave blank to keep current image' : '' ?></div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="tag" value="<?= htmlspecialchars($edit_row['tag'] ?? '') ?>"
                                placeholder="Welcome / Philosophy / Values…" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" value="<?= htmlspecialchars($edit_row['title'] ?? '') ?>"
                                placeholder="Slide heading" required>
                        </div>
                        ...
                        <div class="form-group full">
                            ...
                            <textarea name="description"
                                placeholder="Slide paragraph text (HTML allowed)…"><?= htmlspecialchars($edit_row['description'] ?? '') ?></textarea>
                            <div class="hint">HTML is allowed — you can use &lt;strong&gt;, &lt;em&gt;, &lt;br&gt; tags</div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="btn_text"
                                value="<?= htmlspecialchars($edit_row['btn_text'] ?? 'Learn More →') ?>"
                                placeholder="Learn More →">
                        </div>
                        <div class="form-group">
                            <label>🔗 Button Link</label>
                            <input type="text" name="btn_link"
                                value="<?= htmlspecialchars($edit_row['btn_link'] ?? '#about') ?>"
                                placeholder="#about or samacheer.php#admissions">
                        </div>
                    ...
                    <div style="display:flex;gap:10px;margin-top:20px;">
                        <button type="submit" class="btn btn-primary">
                            <?= $edit_row ? '💾 Save Changes' : '➕ Add Slide' ?>
                        </button>
                        <?php if ($edit_row): ?>
            ...
            <div class="card">
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            <?php while ($row = mysqli_fetch_assoc($slides_res)): ?>
                                <tr>
                                    <td><?= $row['id'] ?></td>
                                    <td>
                                        <img src="../<?= htmlspecialchars($row['image_path']) ?>" class="slide-thumb"
                                            onerror="this.src='https://placehold.co/72x46?text=No+Img'">
                                    </td>
                                    <td>
                                        <strong><?= htmlspecialchars($row['title']) ?></strong><br>
                                        <span
                                            style="color:#94a3b8;font-size:0.78rem;"><?= htmlspecialchars($row['tag']) ?></span>
                                    </td>
                                    <td>
                                        <span style="font-size:0.8rem;"><?= htmlspecialchars($row['btn_text']) ?></span><br>
                                        <span
                                            style="color:#94a3b8;font-size:0.74rem;"><?= htmlspecialchars($row['btn_link']) ?></span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            <a href="slides.php?edit=<?= $row['id'] ?>" class="btn-sm btn-edit">✏️ Edit</a>
                                            <a href="slides.php?action=toggle&id=<?= $row['id'] ?>"
                                                class="btn-sm btn-toggle" title="Show/Hide">👁
                                                <?= $row['is_active'] ? 'Hide' : 'Show' ?></a>
                                            <a href="slides.php?action=delete&id=<?= $row['id'] ?>" class="btn-sm btn-del"
